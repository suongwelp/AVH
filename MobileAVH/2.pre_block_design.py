...
bands = ['Voice_0.1', '0.3', '0.5', '0.7', '0.9', '1.1', '1.3', '1.5', 'Noise_1.7']
...
seq = slab.Trialsequence(conditions=stim_device, n_reps=1)  # 2 repetitions per condition
...
os.chdir(file_path)
with open('seq_exp1_'+ time.strftime("%m_%d_%H_%M")+'.pkl', 'wb') as f:
    pickle.dump(bands, f)


### load seq back in to check results#############################################################################

#file_path = os.path.join(main_path,vp,'seq_exp1.pkl').replace("\\", "/")
#with open(file_path, 'rb') as file:
...
plt.tight_layout()
save_path = os.path.join(main_path,vp,f'{vp}_pre_vocode_exp1_' + time.strftime("%d%m_%H_%M") +'.svg').replace("\\", "/")
